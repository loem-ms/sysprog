...
int exists_file(char *fname) {
    struct stat info;
    int r;
    if ((r =stat(fname, &info)) == -1) return 0;
    return S_ISREG(info.st_mode);
}
/** Checks whether a file is executable. */
int is_file_executable(char *fname) {
...
void detach_children(void) {
    ...
    if (sigaction(SIGCHLD, &act, NULL) == -1) PERROR_DIE("sigaction");
}
...
void send_403(FILE *fout) {
    send_status_body(fout, "403 Forbidden", "<title>403 Forbidden</title><h1>403 Forbidden</h1>\n");
}
void send_404(FILE *fout) {
    send_status_body(fout, "404 Not Found", "<title>404 Not Found</title><h1>404 Not Found</h1>\n");
}
...
void session(int fd, FILE *fout, FILE *fin, char *caddr, int cport) {
    ...
    // request line
    if (fgets(line, sizeof(line), fin) == NULL) return;
    sscanf(line, "%s %s %s", method, uri, version);
    LOG("(%d) method = '%s', uri = '%s', version = '%s'", getpid(), method, uri, version);

    // header
    for (;;) {
        if (fgets(line, sizeof(line), fin) == NULL) {
            LOG("(%d) Illegal headers; disconnected.", getpid());
            return;
        }
        if (strcmp(line, "\r\n") == 0) break;
        LOG("(%d) [Header] %s", getpid(), chomp(line));
    }
    ...
    */
    // Task1 and TaskA
    char fpath[1024];
    strcpy(fpath,"docroot");
    strcat(fpath,uri);
    char *fname = get_filename(uri);
    if (!strcmp(fname,"")){
        strcat(fpath,"index.html");
    }
    if ((exists_file(fpath)) == 0){
        send_404(fout);
    }else if (is_file_executable(fpath)){
        int fd[2];
        pipe(fd);
        if(fork()==0){
            dup2(fd[1],1);
        ...
        close(fd[0]);close(fd[1]);
        fprintf(fout, "HTTP/1.0 200 OK\r\n");
        send_stream(fout,stdin);
    }else{
        fprintf(fout, "HTTP/1.0 200 OK\r\n");
        fprintf(fout, "Content-Type: %s\r\n",get_content_type(get_extension(fpath)));
        fprintf(fout, "\r\n");
        if ((fin = fopen(fpath,"r")) == NULL){
            send_403(fout);
        }else{
            send_stream(fout,fin);
            fclose(fin);
        }
...
void do_session(int fd, char *caddr, int cport) {
    ...
    LOG("(%d) Connection closed.", getpid());
}

void serve() {
    int listfd = socket(AF_INET, SOCK_STREAM, 0);
    if (listfd == -1) PERROR_DIE("socket");
    int optval = 1;
    if (setsockopt(listfd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) == -1) PERROR_DIE("setsockopt");
    ...
    if (bind(listfd, (struct sockaddr *)&saddr, sizeof(saddr)) == -1) PERROR_DIE("bind");
    if (listen(listfd, 10) == -1) PERROR_DIE("listen");
    ...
    for (;;) {
        ...
        if ((cpid = fork()) == -1){
            ...
            if (close(listfd) == -1){
                PERROR_DIE("close listening socket");
            }
